...
void ncomp(int algol, int delay, int *array) {
    ...
    // This is used to have the user provide post-sort input.
    int userinput = 0;

    // Here's where the user loop begins, by the way.
    // this program is a mess, lord.
    while (userinput != 'q') {
        ...
        // Here we store the original array.
        int originalarray[18];
        for(int i = 0; i < 18; i++)
        ...
        mvwprintw(bborder, 7, 24, "Sorting is complete.");
        mvwprintw(bborder, 8, 24, "Press 'q' to quit, and 'a' to change algorithm.");
        wrefresh(bborder);
        wrefresh(stdscr);
