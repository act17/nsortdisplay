...
void bubble(int * array,int size,int delay){
  	struct timespec ts;				//NOTE TO SELF: Fix the tabbing on this file in the future.
	...
	for(int a = 0; a < size; a++){
		for(int b = 1; b < size - a; b++){	//This for statement runs a routine that moves up the highest value.
			...
			currentnum = array[c];		//These are our compared numbers. The value of these operators changes a lot
			nextnum = array[c+1];		//While also being imperitive to the function of the algorithim.

			nanosleep(&ts,&ts);		//This is placed right before the big "if" statement that does the comparison.
				        		//The reason is to create a delay before a major comparison is made.
			
			if(currentnum > nextnum){	//Next three lines can be considered the "swap routine".
				array[c] = nextnum;	//This makes the current - soon to be previous - entry the lower value.
				c++;			//This increases the entry under inspection by one.
				array[c] = currentnum;	//This makes the next - now the current - entry the higher value.
			}
		...
		if(a != (size - 1))
			array[c] = currentnum;		//This garuntees that the set-in-stone final value of the entry is the highest.
