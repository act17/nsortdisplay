...
char delayselections[4][66] = {"Slow: 500 ms","Medium: 250 ms","Fast: 125 ms","Custom Delay"};
...
int delaysel(){
  ...
  while(1){
	...
	wattron(main,A_REVERSE);
      ...
      if(i != 3)  //"Comparisons per Second" will not be printed for the line dedicated to choosing a custom delay.
  ...
  if(delay == 0){			//This checks to see if the user has chosen a custom delay.
    ...
    while(1){
      ...
      wattron(entry,COLOR_PAIR(3));	//This ensures the following informational text is red.
	...
	mvwprintw(entry,1,0,"Error! Delay cannot be 0!");
      ...
      if(delay <= 2147 && delay > 0)	//For some reason, this is the only way to make this work;
	break;				//Not do-while loop.

    }
    
    noecho();
    delay = delay * 1000000;		//This converts the delay in milliseconds(?) to nanoseconds.
  }
